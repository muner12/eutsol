...
export const DropDownInput = ({options}) => {
    ...
    const generateOptions = useCallback((options) => {
        return options.map((item, index) => {
            if (Array.isArray(item.chiledren)) {
                return (
                    <li key={index + 'child'}
                        onClick={() => handleChange(item.chiledren.map((child) => child.value).join(','))}
            ...
            return <li key={index} onClick={() => handleChange(item.value)}
                data-contextmenu='dropdown'
            >{item.value}</li>;
        });
    })
    ...
    return (
        <div className='mb-2' ref={dropdownRef}>
            ...
            className='relative w-80' data-contextmenu='dropdown'>
        <input
        ...
        />
        <SlArrowDown
            className='absolute p-2 text-[1.8rem] top-1/2 right-3  transform -translate-y-1/2 hover:bg-gray-200 text-gray-500 cursor-pointer  rounded-sm'

         />
        <ul className={`absolute ${dropwDownToggle ? 'block' : 'hidden'} mt-2 w-80 border border-gray-200 rounded-lg`}>
